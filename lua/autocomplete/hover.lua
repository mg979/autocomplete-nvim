...
local fancy_floating_markdown = function(contents, opts)
  ...
  local winnr
  if G.docked_hover == 1 then
    if height > G.maximum_size then
      height = G.maximum_size
    elseif height < G.minimum_size then
      height = G.minimum_size
    end
    local row
...
M.autoOpenHoverInPopup = function()
  ...
  if vim.fn.pumvisible() == 1 then
    ...
    if Var.textHover == true and items['selected'] ~= -1 then
